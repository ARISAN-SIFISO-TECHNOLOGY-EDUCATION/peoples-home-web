# Application Memories

> One memory per app. Each captures the *evolution of ideas*: why the app exists, the original
> vision, major design iterations, architectural decisions, rejected alternatives, and future
> direction.
>
> For **live** facts (repo, URL, build config, current status) see the operational
> [`../../ECOSYSTEM.md`](../../ECOSYSTEM.md) and [`../../ROADMAP.md`](../../ROADMAP.md).
>
> For the full 40-app architecture see [`../architecture/01-capability-architecture.md`](../architecture/01-capability-architecture.md).

---

## Memory files (live apps — deep documentation exists)

| # | App | Category | Memory |
|---|---|---|---|
| 1 | **Math Adventure RPG** | Foundations | [`math-adventure-rpg.md`](./math-adventure-rpg.md) |
| 2 | **ReadAfrica** | Foundations | [`read-africa.md`](./read-africa.md) |
| — | **Everyday Foundations** *(v2 — maps to #3/#4/#30)* | Foundations / Empowerment | [`everyday-foundations.md`](./everyday-foundations.md) |

## Memory files (planned apps — blueprint exists)

| # | App | Category | Memory |
|---|---|---|---|
| 3 | **Early Numeracy** | Foundations | [`early-numeracy.md`](./early-numeracy.md) |
| 6 | **Science Sprouts** | Curiosity | [`science-sprouts.md`](./science-sprouts.md) |
| — | **Our World** *(v2 — maps to #10)* | Curiosity | [`our-world.md`](./our-world.md) |
| 11 | **Tech Makers** | Creation | [`tech-makers.md`](./tech-makers.md) |
| — | **SIFISO — The Golden Hand** *(v2 — maps to #14/#26)* | Creation / Empowerment | [`the-golden-hand.md`](./the-golden-hand.md) |
| 16 | **Truth Seekers** | Thinking | [`truth-seekers.md`](./truth-seekers.md) |
| 21 | **Mzansi Money** | Money | [`mzansi-money.md`](./mzansi-money.md) |
| 26 | **Micro Founders** | Empowerment | [`micro-founders.md`](./micro-founders.md) |

World B / platform pieces live in [`../platform/`](../platform/).

---

## All 40 apps (v3 canonical map)

✅ LIVE · 📋 Planned · 🔮 Future

### 1 · Foundations — Build the ability to learn

| # | App | Status |
|---|---|---|
| 1 | Math Adventure RPG | ✅ LIVE |
| 2 | ReadAfrica | ✅ LIVE |
| 3 | Early Numeracy | 🔮 Future |
| 4 | Early Literacy Games | 🔮 Future |
| 5 | Parent Companion | 🔮 Future |

### 2 · Curiosity & Knowledge — Build curiosity

| # | App | Status |
|---|---|---|
| 6 | Science Sprouts | ✅ LIVE |
| 7 | Nature Explorer | 🔮 Future |
| 8 | Space Explorer | 🔮 Future |
| 9 | Human Body Explorer | 🔮 Future |
| 10 | African Discoveries | 🔮 Future |

### 3 · Creation & Technology — Build creators

| # | App | Status |
|---|---|---|
| 11 | Tech Makers | ✅ LIVE |
| 12 | Design Studio | 🔮 Future |
| 13 | Robotics Lab | 🔮 Future |
| 14 | AI Explorer | 🔮 Future |
| 15 | Digital Creator | 🔮 Future |

### 4 · Thinking & Reasoning — Build thinkers

| # | App | Status |
|---|---|---|
| 16 | Truth Seekers | ✅ LIVE |
| 17 | Logic Lab | 🔮 Future |
| 18 | Debate Academy | 🔮 Future |
| 19 | Problem Solver | 🔮 Future |
| 20 | Research Skills | 🔮 Future |

### 5 · Money & Opportunity — Build financial capability

| # | App | Status |
|---|---|---|
| 21 | Mzansi Money | ✅ LIVE |
| 22 | Side Hustle Lab | 🔮 Future |
| 23 | Personal Budget Coach | 🔮 Future |
| 24 | Smart Shopper | 🔮 Future |
| 25 | Financial Wellness | 🔮 Future |

### 6 · Real-World Empowerment — Build independence

| # | App | Status |
|---|---|---|
| 26 | Micro Founders | ✅ LIVE |
| 27 | Career Explorer | 🔮 Future |
| 28 | Leadership Academy | 🔮 Future |
| 29 | Community Builder | 🔮 Future |
| 30 | Life Skills | 🔮 Future |

### 7 · iKhaya — Opportunity Ecosystem *(World B)*

| # | App / Module | Status |
|---|---|---|
| 31 | Opportunity Hub | ✅ LIVE |
| 32 | Skills Passport | 📋 Planned |
| 33 | Job Ready | 📋 Planned |
| 34 | Employer Connect | 📋 Planned |
| 35 | Local Opportunities | 📋 Planned |

### 8 · Community & People *(World B)*

| # | App / Module | Status |
|---|---|---|
| 36 | Parent Hub | 🔮 Future |
| 37 | Teacher Hub | 🔮 Future |
| 38 | Volunteer Hub | 🔮 Future |
| 39 | Community Projects | 🔮 Future |
| 40 | People's Forum | 🔮 Future |

---

## Memory file template

Every deep memory follows this structure:

1. **What it is** — one paragraph.
2. **Category & audience.**
3. **Why it exists** — the gap it fills.
4. **Original vision.**
5. **Design iterations / how the thinking evolved.**
6. **Architectural decisions** (and **rejected alternatives**).
7. **Current state** (links to operational docs).
8. **Future direction.**
9. **Related.**
