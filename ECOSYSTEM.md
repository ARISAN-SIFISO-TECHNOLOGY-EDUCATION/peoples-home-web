...
| **Space Explorer** | Curiosity | `ARISAN-SIFISO-TECHNOLOGY-EDUCATION/Space-Explorer` (private, branch `main`) | ⚠️ **not deployed** (`space-explorer.pages.dev` is taken by an unrelated app — needs a different subdomain) | Vite 6 · React 19 · TS · Tailwind v4 · vite-plugin-pwa | `npm run build` · `dist`. Built in Gemini AI Studio; **brought in-house 2026-07-03** (AI dropped, offline PWA, CSP, real icons, storage bug fixes, new README). See `brain/apps/space-explorer.md`. |
