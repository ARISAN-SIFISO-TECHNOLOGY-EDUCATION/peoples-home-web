...
| **African Discoveries** | Curiosity | `ARISAN-SIFISO-TECHNOLOGY-EDUCATION/african-discoveries` (private, branch `main`) | **https://african-discoveries.pages.dev/** ✅ | Vite 7 · React 19 · TS · Tailwind v4 · wouter · shadcn · vite-plugin-pwa · **pnpm** | `pnpm build` · `dist/public`. Built in Gemini via a Manus/Builder "forge" scaffold; **brought in-house + deployed + linked 2026-07-06** (scaffold stripped, express server dropped, 6 images recovered from the published Manus app, Fredoka self-hosted, strict CSP, offline PWA + true-PNG icons). Node pinned 22 (`.node-version`). See `brain/apps/african-discoveries.md`. |
