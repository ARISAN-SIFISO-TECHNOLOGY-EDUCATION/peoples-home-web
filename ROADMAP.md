...
| Space Explorer | Curiosity | **In-house & deploy-ready — NOT deployed** (2026-07-03). Built in Gemini AI Studio; brought in-house (AI dropped, offline PWA, CSP, real icons, storage bug fixes, new README). Solar-system discovery, ages 3–17, `main` branch. ⚠️ `space-explorer.pages.dev` is taken by an unrelated "GravityDemo" → needs a different Cloudflare project/subdomain, then link on the site. See `brain/apps/space-explorer.md`. |
...
| Early Numeracy | Foundations | **LIVE (installable offline PWA)** — https://early-numeracy.pages.dev/ . Number-sense 3–7. Phase 1 shell (2026-06-30); **audit + security hardening 2026-07-03** (real icons, ErrorBoundary, CSP, zero-external) + linked on the site. Youngest-learner on-ramp below Math Adventure. **Next: Phase 2 real gameplay** (all 5 modules still "Coming soon"). |
