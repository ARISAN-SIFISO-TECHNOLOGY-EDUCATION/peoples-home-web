...
| Rule | Why | Source |
|---|---|---|
| **Use Math Adventure's `useNarration`** (wait for `voiceschanged`, pin an installed voice, isiZulu→English fallback). Never the naive `speak()`. | Science Sprouts shipped silent using `speak()`. **Device-audio is a release gate.** | [`../HISTORY.md`](../HISTORY.md) |
| **Less text + calmer sound for young ages.** | Math Adventure's "Expert Approved" badge was rejected for text volume + disruptive sounds (2026-06-17). | [`../HISTORY.md`](../HISTORY.md) |
